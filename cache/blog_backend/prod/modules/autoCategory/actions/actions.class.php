...
abstract class autoCategoryActions extends sfActions
{
  ...
  public function executeIndex(sfWebRequest $request)
  {
    ...
    if ($request->isXmlHttpRequest())
    {
      ...
      if ($request->hasParameter('search'))
      {
        $partialSearch = $this->getPartial('category/search', array('configuration' => $this->configuration));
      }
      
      if ($request->hasParameter('_reset')) 
      {
        $partialFilter = $this->getPartial('category/filters', array('form' => $this->filters, 'configuration' => $this->configuration));
      }
      
      $partialList = $this->getPartial('category/list', array('pager' => $this->pager, 'sort' => $this->sort, 'helper' => $this->helper));
  ...
  public function executeDelete(sfWebRequest $request)
  {
    ...
    if ($request->isXmlHttpRequest()) {
      ...
      $response['redirectToUrl'] = $this->getController()->genUrl('@category');
  ...
  public function executeToggleBoolean(sfWebRequest $request)
  {
      ...
      Doctrine::getTable('Category')->hasField($field = $request->getParameter('field'))
      && $record = Doctrine::getTable('Category')->find($request->getParameter('pk'))
    );
  ...
  {
    $this->getUser()->setAttribute('category.max_per_page', $maxPerPage, 'admin_module');
  }
  ...
  {
    return $this->getUser()->getAttribute('category.search', null, 'admin_module');
  }

  protected function setSearch($search)
  {
    $this->getUser()->setAttribute('category.search', $search, 'admin_module');
  }
