<div class="sf_admin_list">
 ...
 <?php endif; ?>
  <table cellspacing="0">
        ...
        <tfoot>
          <tr>
            <th colspan="5">
              ...
              <?php include_partial('category/pagination_list_select', array( 'pager' => $pager)) ?>
                <?php if ($pager->haveToPaginate()): ?>
	...
	<tbody class='{toggle_url: "<?php echo url_for($helper->getRouteArrayForAction('toggleBoolean', 'category')) ?>"}'>
          <?php foreach ($pager->getResults() as $i => $category): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $category->getPrimaryKey() ?>}">
                  <?php include_partial('category/list_td_batch_actions', array('category' => $category, 'helper' => $helper, 'key' => $i )) ?>
