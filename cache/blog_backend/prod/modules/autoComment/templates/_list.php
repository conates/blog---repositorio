<div class="sf_admin_list">
  ...
  <div class="paginate_top">
    ...
    <?php include_partial('comment/pagination', array('pager' => $pager)) ?>
  </div>
 <?php endif; ?>
  <table cellspacing="0">
    <thead>
      <tr>
            ...
            <label class="" for="sf_admin_list_batch_checkbox"></label>
                  ...
                  <?php include_partial('comment/list_th_tabular', array('sort' => $sort)) ?>
        ...
        <tfoot>
          <tr>
            <th colspan="12">
              ...
              <?php include_partial('comment/pagination_list_select', array( 'pager' => $pager)) ?>
                <?php if ($pager->haveToPaginate()): ?>
	...
	<tbody class='{toggle_url: "<?php echo url_for($helper->getRouteArrayForAction('toggleBoolean', 'comment')) ?>"}'>
          <?php foreach ($pager->getResults() as $i => $comment): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $comment->getPrimaryKey() ?>}">
                  <?php include_partial('comment/list_td_batch_actions', array('comment' => $comment, 'helper' => $helper, 'key' => $i )) ?>
                  <?php include_partial('comment/list_td_tabular', array('comment' => $comment)) ?>
                  <?php include_partial('comment/list_td_actions', array('comment' => $comment, 'helper' => $helper)) ?>
              </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
