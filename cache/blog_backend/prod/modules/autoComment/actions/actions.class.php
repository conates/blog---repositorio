...
require_once(dirname(__FILE__).'/../lib/BaseCommentGeneratorConfiguration.class.php');
require_once(dirname(__FILE__).'/../lib/BaseCommentGeneratorHelper.class.php');
 ...
 */
abstract class autoCommentActions extends sfActions
{
  public function preExecute()
  {
    $this->configuration = new commentGeneratorConfiguration();
    ...
    $this->helper = new commentGeneratorHelper();
  }
  ...
  public function executeDelete(sfWebRequest $request)
  {
    ...
    if ($request->isXmlHttpRequest()) {
      ...
      $response['redirectToUrl'] = $this->getController()->genUrl('@comment');
  ...
  public function executeToggleBoolean(sfWebRequest $request)
  {
      ...
      Doctrine::getTable('Comment')->hasField($field = $request->getParameter('field'))
      && $record = Doctrine::getTable('Comment')->find($request->getParameter('pk'))
    );
  ...
  {
    return $this->getUser()->getAttribute('comment.filters', $this->configuration->getFilterDefaults(), 'admin_module');
  }

  protected function setFilters(array $filters)
  {
    return $this->getUser()->setAttribute('comment.filters', $filters, 'admin_module');
  }
  ...
  {
    $this->getUser()->setAttribute('comment.page', $page, 'admin_module');
  }

  protected function getPage()
  {
    return $this->getUser()->getAttribute('comment.page', 1, 'admin_module');
  }
  ...
  {
    $this->getUser()->setAttribute('comment.max_per_page', $maxPerPage, 'admin_module');
  }
  ...
  {
    if (null !== $sort = $this->getUser()->getAttribute('comment.sort', null, 'admin_module'))
    {
      return $sort;
    }
    ...
    return $this->getUser()->getAttribute('comment.sort', null, 'admin_module');
  }
  ...
  protected function setSort(array $sort)
  {
    ...
    $this->getUser()->setAttribute('comment.sort', $sort, 'admin_module');
  }

  protected function isValidSortColumn($column)
  {
    return Doctrine_Core::getTable('Comment')->hasColumn($column);
  }
  ...
  protected function processSearchQuery(Doctrine_Query $query, $search)
  {
    ...
    $table = Doctrine_Core::getTable('Comment');
  ...
  {
    return $this->getUser()->getAttribute('comment.search', null, 'admin_module');
  }

  protected function setSearch($search)
  {
    $this->getUser()->setAttribute('comment.search', $search, 'admin_module');
  }
